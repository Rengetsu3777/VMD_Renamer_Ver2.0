...
VMDRenamer::VMDRenamer(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::VMDRenamer)
{
    ui->setupUi(this);
}

VMDRenamer::~VMDRenamer()
{
    delete ui;
}

void VMDRenamer::on_pushButton_released()
{
...
void VMDRenamer::openMotionFile()
{
...
void VMDRenamer::on_findBoneFileButton_released()
{
    QString filePath = QFileDialog::getOpenFileName();
...
void VMDRenamer::on_findmotionFileButton_released()
{
...
void VMDRenamer::on_runButton_released()
{
    ...
    std::tuple<int, std::string> convertResult = mainVmdRenamer(vmdPath.c_str(), filePath.c_str(), transformScale);
    int convertResultFlag = std::get<0>(convertResult);
    std::string convertResultDialog = std::get<1>(convertResult);

    if(convertResultFlag) {
        QString logPathQString = QString::fromStdString(printLogContent(log_path));
        std::cout << logPathQString.toStdString() << std::endl;

        switch(convertResultFlag) {
        case -1:
            ui->resultLabel->setText("変換失敗。ログファイルを開けません。");
            dialogString = "変換失敗。ログファイルを開けません。";
            break;
        case 1:
            ui->resultLabel->setText("変換失敗。csvファイルを開けません。");
            dialogString = "変換失敗。csvファイルを開けません。";
            break;
        case 2:
            ui->resultLabel->setText("変換失敗。csvファイル内のボーン名が不正です。詳細はログをご確認ください。");
            dialogString = "変換失敗。csvファイル内のボーン名が不正です。詳細はログをご確認ください。";
            break;
        case 3:
            ui->resultLabel->setText("変換失敗。vmdファイルを開けません。");
            dialogString = "変換失敗。vmdファイルを開けません。";
            break;
        default:
            ui->resultLabel->setText("変換失敗。予期せぬエラーが発生しました。");
            dialogString = "変換失敗。予期せぬエラーが発生しました。";
        }
...
/*
    ...
    Dialog widgetDialog;
    widgetDialog.setDialogText(dialogString);
    widgetDialog.show();

    return a.exec();
*/
    dialogInstance->setDialogText(QString::fromStdString(convertResultDialog));
    dialogInstance->show();
}

void VMDRenamer::on_actionOpen_log_file_triggered()
{
...
void VMDRenamer::on_actionDocument_triggered()
{
    ShellExecute(
        GetDesktopWindow(),
        L"open",
...
void VMDRenamer::setDialogWindow(Dialog* dialog) {
